...
export class LoansService {
	...
	async create( dto: CreateLoansDTO ): Promise<LoansEntity | string> {
		const target = await this.clientsRepo.findOne( {
		...
		loan.total_interest =
			Math.round( dto.requested_amount * dto.interest_rate * 100 ) / 100
		loan.total_loan =
			Math.round( ( dto.requested_amount + loan.total_interest ) * 100 ) / 100
		loan.monthly_interest =
			Math.round( ( loan.total_interest / dto.term_duration ) * 100 ) / 100
		loan.monthly_payment =
			Math.round( ( dto.requested_amount / dto.term_duration ) * 100 ) / 100
